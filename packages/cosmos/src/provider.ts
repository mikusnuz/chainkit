  ...
  implements ChainProvider, ContractCapable, TokenCapable, SubscriptionCapable
{
  ...
  /**
   * Get the account sequence number (nonce) for a Cosmos address.
   */
  async getNonce(address: Address): Promise<number> {
    try {
      const data = await this.lcdGet<{ account?: { sequence?: string } }>(
        `/cosmos/auth/v1beta1/accounts/${address}`,
      )
      return parseInt(data.account?.sequence ?? '0', 10)
    } catch {
      return 0
    }
  }
