...
function slip0010DerivePath(seed: Uint8Array, path: string): Uint8Array {
  if (!BIP44_PATH_REGEX.test(path)) {
    throw new ChainKitError(
      ...
      `Invalid derivation path: "${path}". Expected format: m/44'/637'/0'/0'/0'`,
    )
  }
...
/**
 * Aptos signer implementing the ChainSigner interface.
 * Uses ED25519 keys with SLIP-0010 derivation and SHA3-256 address hashing.
...
export class AptosSigner implements ChainSigner {
  ...
  /**
   * Get the Aptos address for a given private key.
   * Aptos address = SHA3-256(ED25519 public key || 0x00) where 0x00 is the single-key auth scheme.
  ...
  getAddress(privateKey: HexString): Address {
    ...
    // The 0x00 suffix is the single-key ED25519 authentication key scheme identifier
    const authKeyInput = new Uint8Array(publicKey.length + 1)
    authKeyInput.set(publicKey, 0)
    authKeyInput[publicKey.length] = 0x00

    const hash = sha3_256(authKeyInput)
  ...
  async signTransaction(params: SignTransactionParams): Promise<HexString> {
    ...
    if (!tx.data) {
      throw new ChainKitError(
        ...
        'Transaction data (serialized message) is required for Aptos signing',
      )
    }
