...
import type { ZkspaceFeeDetail } from './types.js'
...
/**
 * Configuration for the ZKSpace provider.
 * Extends the standard RPC config with an optional REST API base URL
 * for ZKSpace-specific L2 operations.
 */
export interface ZkspaceProviderConfig extends RpcManagerConfig {
  /** Base URL for ZKSpace REST API (L2-specific endpoints). */
  restApiUrl?: string
}

/**
 * ZKSpace provider implementing ChainProvider, ContractCapable,
 * TokenCapable, and SubscriptionCapable interfaces.
 *
 * Uses EVM JSON-RPC for standard operations and an optional ZKSpace
 * REST API for L2-specific features (fee estimation, deposit status).
 */
export class ZkspaceProvider
  implements ChainProvider, ContractCapable, TokenCapable, SubscriptionCapable
{
  private readonly rpc: RpcManager
  private readonly restApiUrl: string | null

  constructor(config: ZkspaceProviderConfig) {
    this.rpc = new RpcManager(config)
    this.restApiUrl = config.restApiUrl ?? null
  }

  // ------- ChainProvider -------

  /**
   * Get the ETH balance of an address on ZKSpace L2.
   */
   ...
   * Uses the ZKSpace REST API if available (returning L2 fee detail),
   * otherwise falls back to standard EVM EIP-1559 fee estimation.
   */
  ...
  async getChainInfo(): Promise<ChainInfo> {
    ...
    // Map known ZKSpace chain IDs
    const chainNames: Record<number, { name: string; testnet: boolean }> = {
      13: { name: 'ZKSpace Mainnet', testnet: false },
      133: { name: 'ZKSpace Testnet', testnet: true },
    }

    const info = chainNames[chainId] ?? {
      name: `ZKSpace Chain ${chainId}`,
      testnet: chainId > 100,
    }
   ...
   * ZKSpace L2 produces blocks faster than L1, so poll every ~5 seconds.
   */
   ...
   * Polls every ~5 seconds and checks new blocks for matching transactions.
   */
  ...
  // ------- ZKSpace-specific L2 methods -------

  /**
   * Estimate L2 fees using the ZKSpace REST API.
   * Returns both the L2 execution fee and the L1 data/gas fee.
   * Falls back to a default if no REST API is configured.
   */
  async estimateL2Fee(): Promise<ZkspaceFeeDetail> {
    if (this.restApiUrl) {
      try {
        const response = await fetch(`${this.restApiUrl}/fee`)
        if (response.ok) {
          const data = await response.json() as { l2Fee?: string; l1GasFee?: string }
          return {
            l2Fee: data.l2Fee ?? '0',
            l1GasFee: data.l1GasFee ?? '0',
          }
        }
      } catch {
    ...
    // Fallback: use standard gas estimation via JSON-RPC
    const gasPrice = await this.rpc.request<string>('eth_gasPrice', [])
    const gasPriceWei = hexToBigInt(gasPrice)
    const defaultGas = 21000n
    const l2Fee = (gasPriceWei * defaultGas).toString()
  ...
  /**
   * Check the status of an L1 -> L2 deposit.
   * Requires the ZKSpace REST API to be configured.
   * @param l1TxHash - The L1 transaction hash of the deposit
   * @returns Deposit status string or null if not found / REST API unavailable
   */
  async getDepositStatus(l1TxHash: string): Promise<string | null> {
    if (!this.restApiUrl) {
      return null
    }

    try {
      const response = await fetch(`${this.restApiUrl}/deposit/${l1TxHash}/status`)
      if (response.ok) {
