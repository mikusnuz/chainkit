...
describe('ZkspaceProvider', () => {
  let mockFetch: ReturnType<typeof vi.fn>
  let provider: ZkspaceProvider

  beforeEach(() => {
    mockFetch = vi.fn()
    vi.stubGlobal('fetch', mockFetch)
    provider = new ZkspaceProvider({ endpoints: ['http://rpc.test'] })
  })
  ...
  describe('getChainInfo', () => {
    it('should return chain info for ZKSpace mainnet', async () => {
      // eth_chainId (13 = ZKSpace Mainnet)
      mockFetch.mockResolvedValueOnce(createMockResponse('0xd'))
      // eth_blockNumber
    ...
    it('should return chain info for ZKSpace testnet', async () => {
      // eth_chainId (133 = ZKSpace Testnet)
      mockFetch.mockResolvedValueOnce(createMockResponse('0x85'))
      // eth_blockNumber
      mockFetch.mockResolvedValueOnce(createMockResponse('0x100'))

      const info = await provider.getChainInfo()

      expect(info.name).toBe('ZKSpace Testnet')
    ...
    it('should handle unknown chain IDs', async () => {
      ...
      const info = await provider.getChainInfo()
      expect(info.name).toBe('ZKSpace Chain 1337')
    })
  })
  ...
  describe('estimateL2Fee', () => {
    it('should fall back to gas price estimation when no REST API is configured', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse('0x3b9aca00')) // 1 gwei gas price

      const fee = await provider.estimateL2Fee()

      expect(getMethodFromCall(0)).toBe('eth_gasPrice')
      expect(fee.l2Fee).toBe('21000000000000') // 1 gwei * 21000
      expect(fee.l1GasFee).toBe('0')
    })

    it('should use REST API when configured', async () => {
      const providerWithRest = new ZkspaceProvider({
        endpoints: ['http://rpc.test'],
        restApiUrl: 'http://rest.test',
      })

      // Mock the REST API response
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ l2Fee: '50000', l1GasFee: '10000' }),
      } as unknown as Response)

      const fee = await providerWithRest.estimateL2Fee()

      expect(fee.l2Fee).toBe('50000')
      expect(fee.l1GasFee).toBe('10000')
    })
  })

  describe('getDepositStatus', () => {
    it('should return null when no REST API is configured', async () => {
      const status = await provider.getDepositStatus('0xtxhash')
      expect(status).toBeNull()
    })

    it('should query REST API for deposit status', async () => {
      const providerWithRest = new ZkspaceProvider({
        endpoints: ['http://rpc.test'],
        restApiUrl: 'http://rest.test',
      })
      ...
      const status = await providerWithRest.getDepositStatus('0xdeposithash')

      expect(status).toBe('confirmed')
      expect(mockFetch).toHaveBeenCalledWith('http://rest.test/deposit/0xdeposithash/status')
    })

    it('should return null on REST API error', async () => {
      const providerWithRest = new ZkspaceProvider({
        endpoints: ['http://rpc.test'],
        restApiUrl: 'http://rest.test',
      })
      ...
      const status = await providerWithRest.getDepositStatus('0xnotfound')
      expect(status).toBeNull()
    })
  })
})
