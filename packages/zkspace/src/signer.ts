...
/** Default BIP44 HD path for ZKSpace (EVM-compatible). */
export const ZKSPACE_DEFAULT_PATH = "m/44'/60'/0'/0/0"
...
/**
 * ZKSpace signer implementing the ChainSigner interface.
 * ZKSpace is an EVM-compatible L2 (ZK-rollup), so signing logic
 * is identical to Ethereum: BIP39/BIP32 key derivation, EIP-191
 * message signing, and EIP-1559 (type 2) / legacy transaction signing.
 */
export class ZkspaceSigner implements ChainSigner {
  /**
  ...
  /**
   * Get the ZKSpace address for a given private key.
   * Returns an EIP-55 checksummed address (0x + 40 hex chars).
   */
  ...
  async signTransaction(tx: UnsignedTx, privateKey: HexString): Promise<HexString> {
    ...
    if (isEip1559) {
      ...
      const fields: Uint8Array[] = [
        ...
        value,
        data,
        new Uint8Array([]), // empty access list placeholder
      ]
