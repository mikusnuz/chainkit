...
function slip0010DerivePath(seed: Uint8Array, path: string): Uint8Array {
  if (!BIP44_PATH_REGEX.test(path)) {
    throw new ChainKitError(
      ...
      `Invalid derivation path: "${path}". Expected format: m/44'/784'/0'/0'/0'`,
    )
  }
...
export class SuiSigner implements ChainSigner {
   ...
   * Default Sui path: m/44'/784'/0'/0'/0'
  ...
  async signTransaction(tx: UnsignedTx, privateKey: HexString): Promise<HexString> {
    ...
    if (!tx.data) {
      throw new ChainKitError(
        ...
        'Transaction data (serialized transaction bytes) is required for Sui signing',
      )
    }
