...
function slip0010DerivePath(seed: Uint8Array, path: string): Uint8Array {
  if (!BIP44_PATH_REGEX.test(path)) {
    throw new ChainKitError(
      ...
      `Invalid derivation path: "${path}". Expected format: m/44'/397'/0'`,
    )
  }
...
/**
 * NEAR signer implementing the ChainSigner interface.
...
export class NearSigner implements ChainSigner {
  ...
  /**
   * Get the NEAR implicit account ID for a given private key.
   * Returns the hex-encoded ED25519 public key (64 hex characters).
   */
  getAddress(privateKey: HexString): Address {
    ...
    // NEAR implicit account = hex-encoded public key (64 hex chars, no 0x prefix)
    ...
    try {
      ...
      if (!tx.data) {
        throw new ChainKitError(
          ...
          'Transaction data (serialized message) is required for NEAR signing',
        )
      }
