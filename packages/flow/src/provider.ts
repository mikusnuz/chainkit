  ...
  implements ChainProvider, ContractCapable, TokenCapable, SubscriptionCapable
{
   ...
   * Reads the first key's sequence_number from the account.
   */
  async getNonce(address: Address): Promise<number> {
    ...
    try {
      const account = await this.restGet<{
        keys: Array<{ sequence_number: string }>
      }>(`/v1/accounts/${cleanAddress}?block_height=sealed`)

      if (account.keys && account.keys.length > 0) {
        return parseInt(account.keys[0].sequence_number, 10) || 0
      }
    } catch {
      // Fall through
    }
  ...
  /**
   * Get balances for multiple fungible tokens in parallel.
   */
